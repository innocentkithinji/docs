...
The widget uses a `MutationObserver` to automatically detect forms added to the page after initial load. If you dynamically insert a form with `zephlo-tool-name`, it will be picked up automatically.
...
```typescript
import { FormScanner } from "zephlo";
